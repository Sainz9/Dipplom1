...
<body class="antialiased flex flex-col min-h-screen overflow-x-hidden">
    ...
    <main class="pt-20 flex-grow">
        ...
        <div class="py-12 px-4 md:px-8 lg:px-12 space-y-16 -mt-10 relative z-10">
            ...
            @php
                if (!function_exists('renderGameCard')) {
                    function renderGameCard($game, $customBorder = '') {
                        ...
                        // Үнэгүй шалгах (Price=0 or Tag='FreeGame')
                        ...
                        elseif ($tagText) {
                            ...
                            switch($tagText) {
                                ...
                                case 'Эрэлттэй': $badgeClass = 'bg-orange-500 text-white border-orange-400'; $icon = '⚡'; break;
                        ...
                        if ($isComingSoon) {
                            ...
                            if((is_numeric($game->price) && $game->price == 0) || $game->tag == 'FreeGame') {
                                ...
                                if($game->sale_price) {
                                    $priceHTML = '<div class="flex flex-col leading-none"><span class="text-[10px] text-gray-500 line-through">' . number_format($game->price) . '₮</span><span class="text-green-400 font-bold text-sm">' . number_format($game->sale_price) . '₮</span></div>';
                                } else {
                                    $priceHTML = '<span class="text-gray-300 font-bold text-sm">' . number_format($game->price) . '₮</span>';
                                }
                            } else {
                ...
                <div class="swiper swiperComing !overflow-visible !pb-10">
                    <div class="swiper-wrapper">
                        @foreach($comingSoonList as $game)
                            {{ renderGameCard($game, 'hover:border-purple-500') }}
                        @endforeach
                    </div>
                </div>
            </section>
            ...
            {{-- 2. SALE SECTION --}}
            @php
                $onSaleGames = $games->filter(function($game) {
                    return !is_null($game->sale_price) && $game->sale_price > 0;
                });
            @endphp
                ...
                <div class="swiper swiperSale !overflow-visible !pb-10">
                    <div class="swiper-wrapper">
                        @foreach($onSaleGames as $game)
                            {{ renderGameCard($game, 'hover:border-red-600') }}
                        @endforeach
                    </div>
                </div>
            </section>
            ...
            {{-- 3. FREE TO PLAY SECTION (NEW!) --}}
            @php
                $freeGames = $games->filter(function($game) {
                    // Үнэ нь 0 эсвэл Tag нь 'FreeGame' бол үнэгүй гэж үзнэ
                    return (is_numeric($game->price) && $game->price == 0) || $game->tag === 'FreeGame';
                });
            @endphp
                ...
                <div class="swiper swiperFree !overflow-visible !pb-10">
                    <div class="swiper-wrapper">
                        @foreach($freeGames as $game)
                            {{ renderGameCard($game, 'hover:border-green-500') }}
                        @endforeach
                    </div>
                </div>
            </section>
                    ...
                    <div class="swiper swiper-{{ $key }} !overflow-visible !pb-10">
                        <div class="swiper-wrapper">
                            @foreach($filteredGames as $game)
                                {{ renderGameCard($game, $style['border']) }}
                            @endforeach
                        </div>
                    </div>
                </section>
